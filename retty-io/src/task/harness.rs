    ...
    S: Schedule,
{
    ...
    pub(super) fn poll(self) {
        trace!("retty-io DEBUG[Harness]:: poll");
    ...
    pub(super) fn dealloc(self) {
        trace!("retty-io DEBUG[Harness]:: dealloc");
    ...
    pub(super) fn finish(self, val: <T as Future>::Output) {
        trace!("retty-io DEBUG[Harness]:: finish");
        self.header().state.transition_to_running();
    ...
    pub(super) fn try_read_output(self, dst: &mut Poll<T::Output>, waker: &Waker) {
        trace!("retty-io DEBUG[Harness]:: try_read_output");
    ...
    pub(super) fn drop_join_handle_slow(self) {
        trace!("retty-io DEBUG[Harness]:: drop_join_handle_slow");
    ...
    pub(super) fn wake_by_val(self) {
        trace!("retty-io DEBUG[Harness]:: wake_by_val");
        let owner_id = self.header().owner_id;
        if is_remote_task(owner_id) {
            // send to target thread
            trace!("retty-io DEBUG[Harness]:: wake_by_val with another thread id");
            #[cfg(feature = "sync")]
            {
    ...
    pub(super) fn wake_by_ref(&self) {
        trace!("retty-io DEBUG[Harness]:: wake_by_ref");
        let owner_id = self.header().owner_id;
        if is_remote_task(owner_id) {
            // send to target thread
            trace!("retty-io DEBUG[Harness]:: wake_by_ref with another thread id");
            #[cfg(feature = "sync")]
            {
                use crate::task::waker::raw_waker;
                let waker = raw_waker::<T, S>(self.cell.cast::<Header>().as_ptr());
    ...
    pub(super) fn drop_reference(self) {
        trace!("retty-io DEBUG[Harness]:: drop_reference");
