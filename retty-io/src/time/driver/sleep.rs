...
///
/// ```
/// use retty_io::time::{sleep_until, Duration, Instant};
///
/// #[retty_io::main(timer_enabled = true)]
/// async fn main() {
...
///
/// ```
/// use retty_io::time::{sleep, Duration};
///
/// #[retty_io::main(timer_enabled = true)]
/// async fn main() {
...
pin_project! {
    ...
    /// ```
    ///
    /// Use with [`select!`]. Pinning the `Sleep` with [`retty_io::pin!`] is
    /// necessary when the same `Sleep` is selected on multiple times.
    /// ```no_run
    /// use retty_io::time::{self, Duration, Instant};
    ///
    /// #[retty_io::main(timer_enabled = true)]
    /// async fn main() {
...
impl Sleep {
    ...
    ///
    /// ```
    /// use retty_io::time::{Duration, Instant};
    ///
    /// # #[retty_io::main(timer_enabled = true)]
    /// # async fn main() {
    /// let sleep = retty_io::time::sleep(Duration::from_millis(10));
    /// retty_io::pin!(sleep);
    ///
    /// sleep
    ///     .as_mut()
